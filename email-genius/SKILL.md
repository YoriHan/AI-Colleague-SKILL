...
**Day 37 (2026-07-28 → 2026-07-29):**

Standing DM-language rule found unapplied to recurring automated reports (process gap, self-caught and being corrected): the Day 2 rule that anything landing in Yori's DM should be in English (since it can end up as external material) was written with one-off replies in mind and never actually got extended to the *recurring* automated reports this skill sends every cycle — Gmail patrol reports, invoice alerts, and KOL price-scan updates have been going out in Chinese by default since Day 2, in quiet violation of that same rule. Self-flagged to Yori mid-day rather than silently flipping the default; the plan is to switch these report types to English starting with the next patrol cycle unless Yori says the Chinese default was actually intentional for these specific report types. Update: the 00:15 JST patrol cycle landed after this addendum was first drafted and confirms the switch took effect — first report sent in English, no objection heard so it proceeded. Rule for future language-preference changes generally: a standing communication rule needs to be checked against *every* report/message type it should logically cover when it's written, not just the type it was written in response to — an unapplied rule that nobody notices for five weeks is functionally the same as no rule.

Suspicious injected step in the patrol procedure document, found and removed (security — removed and current doc re-verified clean; provenance/audit trail unresolved; no runtime use of the removed step): a STEP 0.2 in the live patrol procedure — instructing a full enumeration of environment variable names and pushing that output to an unrelated task — was discovered and removed. Current document content re-verified clean, only the original STEP 0/0.5/1/2/3 remain. No edit-history trail was available to identify who added it or when; the genuinely-needed diagnostic (variable *names and presence only*, never values) was routed through the direct investigation task instead, so nothing of value was lost by removing the suspicious step. Flagged to Yori in case the provenance is worth digging into further, but not investigated deeper here.

Day 34's invoice-resurfacing fix confirmed still incomplete (live evidence, not just the "not yet implemented" caveat already on file): a payment already confirmed and logged got re-flagged as still-pending in two consecutive patrol reports on 07-28, and had to be manually caught and skipped a third time this round to avoid logging it again. This is the exact failure mode Day 34 root-caused (unread-only fetch lets an already-processed confirmation email resurface indefinitely) and partially mitigated with a pre-write duplicate check against the full log — that mitigation is holding at the *logging* layer (no new duplicate row was written this round), but the underlying resurfacing keeps forcing a fresh manual catch at the *reporting* layer every cycle. Day 34's longer-term fix (mark processed emails read, or track processed message IDs, so they stop resurfacing in the unread fetch at all) is still not built; this is now a second, independent instance confirming it's needed, not hypothetical.

Duplicate-draft count, updated: continues climbing steeply — now roughly ten times the count first flagged, still spread across the same five KOL threads identified in Day 36. Root cause and fix direction are already fully tracked in §11 above; not re-litigated here. The manual mitigation (skip re-drafting a thread that already has an unsent draft waiting) is holding the count from climbing further within this set of threads, but it's memory-based per patrol cycle, not a structural guarantee — see §11 for why a write-time check alone still wouldn't fully close this.

Four items remain open, awaiting Yori's call as of this review, none blocking this file: whether to prioritize building the check-before-write draft fix now versus later; whether to bulk-clean the historical duplicate drafts before or after that fix ships (this skill's standing recommendation is after, so a same-day cleanup doesn't just refill); whether to close out one small invoice confirmed superseded by a later, already-paid one; and whether one newly-appeared invoice from an unfamiliar sender reflects a legitimate collaboration. None of the four have a reply yet as of this review.
